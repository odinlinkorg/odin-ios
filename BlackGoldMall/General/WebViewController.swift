...
//        for (index, item) in navigationController!.viewControllers.enumerated() {
//            if item.isKind(of: ScannerVC.self) {
//                vcs?.remove(at: index)
//            }
//        }
...
//    隐私协议
      func getPrivacyAgreement(){
          NetworkManager<BaseModel>().requestModel(API.shopCommonGetPrivacyAgreement, completion: { (response) in
              if let dic = response?.dataDict{
                  let title = dic["textName"] as! String
                  let textValue = dic["textValue"] as! String
                self.webView.loadHTMLString(textValue, baseURL: nil)
              }
          }) { (error) in
